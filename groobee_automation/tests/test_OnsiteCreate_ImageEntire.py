...
class TestCreateCampaign(BaseClass):
    # 테스트 변수 설정
    TEST_TITLE_DATA = "[QA][자동화][PC] 이미지전체"
    TEST_DES_DATA = "[QA][자동화][PC] 이미지전체_상세설명"
    TEST_TAG_DATA = "자동화태그"
    ...
    def test11(self, driver):
        ...
        oncam.input_textarea(oncam.clk_area_setting_url, self.TEST_URL)
        assert oncam.get_text(oncam.clk_area_setting_url) == self.TEST_URL, "[Fail] 이미지 전체 설정 실패"
    ...
    def test15(self, driver):
        ...
        assert oncam.get_text(oncam.btn_url_blank) == self.TEST_URL, "[Fail] 버튼 입력 실패"
    ...
    def test17(self, driver):
        ...
        assert oncam.is_camplaign_display(self.TEST_TITLE_DATA), "[Fail] 캠페인 생성 실패"
