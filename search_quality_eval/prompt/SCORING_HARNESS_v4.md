...
```json
{
  "session_id":   "sess-1-000aa11f",  // 세션 ID
...
```json
{
  "session_id":   "sess-M01-000aa11f",  // 세션 ID
...
| 세션 | `session_id` | 모노스페이스, 케이스 셀 아래 소형 표시 |
| 카테고리 | 케이스 유형 분류 | |
...
**멀티턴 표** — 단일턴과 동일한 인라인 방식. 시나리오 1행 안에 각 턴을 서브행으로 표시.

| 컬럼 | report JSON 소스 | 설명 |
|---|---|---|
| 시나리오 | `scenario_id` | 모노스페이스 강조 |
| 세션 | `session_id` | 모노스페이스, 케이스 셀 아래 소형 표시 |
| 카테고리 | 케이스 유형 분류 | |
| **턴별 질의 / 답변 · 액션칩** | 각 턴의 `query` + `answer_text` + `keyword_relevance_check.keywords` | T1/T2/T3 서브행 인라인 ← 7-3 참조 |
| 판정 | `evaluation.final_judgement` (1턴 기준) | PASS / FAIL 뱃지 |
| 위반 | 위반 규칙 코드 | R1~R5 / drift |

> **멀티턴 상품 버튼**: 단일턴과 마찬가지로 각 서브행 우측에 표시. 턴별 상품 수가 다르므로 각각 독립적으로 렌더링.

### 7-3. 질의 / 답변 · 액션칩 셀 (인라인 3단 구성)

단일턴과 멀티턴 모두 **동일한 인라인 구조**를 사용한다. 멀티턴은 이 구조를 턴 수만큼 서브행으로 반복한다.
...
#### 멀티턴 셀 구조 (시나리오 1행 안에 T1/T2/T3 서브행)
...
| ② 액션칩 | `<div class="q-chips">` + `<span class="q-chip">` | `keyword_relevance_check.keywords` | 배열 1개 이상일 때만 |
| ③ 답변 | `<div class="q-answer collapsed" id="ans-{case_id}">` | `answer_text` | 텍스트 있을 때만 |
| 토글 | `<span class="q-answer-toggle">` | — | 답변 있을 때만 |
| 상품 버튼 | `<button class="btn-products">` | `product_scores` | 단일턴: 셀 마지막 컬럼 / 멀티턴: 서브행 우측 |

> `id` 속성: 단일턴은 `ans-{query_id}` (예: `ans-Q001`), 멀티턴은 `ans-{case_id}` (예: `ans-MT001-T2`).
> 멀티턴 시나리오 내 턴 간 id 충돌 방지를 위해 반드시 `case_id` 사용.
...
<td class="q-cell">
  ...
  <div class="q-answer collapsed" id="ans-Q001">ランニングを始められる方に…</div>
  <span class="q-answer-toggle" onclick="toggleAns('ans-Q001', this)">▾ 더보기</span>
</td>
```

#### HTML 렌더링 예시 — 멀티턴 (시나리오 1행)
...
<td class="q-cell">
  <div class="mt-turns">
    ...
    <!-- T1 서브행 -->
    <div class="mt-turn-row">
      <span class="mt-turn-num">T1</span>
      <div class="mt-turn-body">
        <div class="q-text">子どもの少年野球用にグローブを探しています。</div>
        <div class="q-chips">
          <span class="q-chip">初心者向けジュニアグローブ</span>
          <span class="q-chip">投手用ジュニアグローブ</span>
        </div>
        <div class="q-answer collapsed" id="ans-MT001-T1">ジュニア用野球グローブをお探しですね。…</div>
        <span class="q-answer-toggle" onclick="toggleAns('ans-MT001-T1', this)">▾ 더보기</span>
      </div>
      <div class="mt-turn-product">
        <button class="btn-products" onclick="openProducts('MT001-T1', &quot;子どもの少年野球用に…&quot;)">상품 20건</button>
      </div>
    </div>

    <!-- T2 서브행 -->
    <div class="mt-turn-row">
      <span class="mt-turn-num">T2</span>
      <div class="mt-turn-body">…</div>
      <div class="mt-turn-product">
        <button class="btn-products" onclick="openProducts('MT001-T2', &quot;ポジションはピッチャーで…&quot;)">상품 6건</button>
      </div>
    </div>
...
검색 결과 상품을 모달 팝업으로 확인한다. 단일턴/멀티턴 모두 동일.

#### 버튼 규칙
- `product_scores` 배열이 1건 이상: **`상품 N건`** 버튼 (파란 테두리, 클릭 가능)
- `product_scores` 빈 배열 또는 결과 0건: **`0건`** 버튼 (disabled)

단일턴은 행 마지막 컬럼 셀에, 멀티턴은 각 서브행의 `.mt-turn-product` 영역에 렌더링.
...
<!-- 멀티턴 각 턴 상품 버튼 -->
<button class="btn-products"
        onclick="openProducts('MT001-T1', &quot;子どもの少年野球用に…&quot;)">상품 20건</button>
```

### 7-6. 스타일 가이드 (CSS 변수 기반)

```css
/* ── 질의/답변·칩 인라인 셀 (단일턴/멀티턴 공통) ── */
.q-cell { max-width: 380px; }

.q-text { color: var(--ink2); margin-bottom: 6px; line-height: 1.55; }

.q-chips { display: flex; flex-wrap: wrap; gap: 4px; margin-bottom: 6px; }
.q-chip {
  display: inline-block; padding: 2px 8px; border-radius: 999px;
  font-size: 10.5px; font-weight: 500;
  background: var(--bg2); color: var(--accent2); border: 1px solid var(--line2);
  white-space: nowrap;
}

.q-answer {
  font-size: 11.5px; line-height: 1.65; color: var(--ink2);
  padding: 6px 8px; background: var(--bg);
  border-left: 2px solid var(--line2); border-radius: 0 4px 4px 0;
  white-space: pre-wrap; word-break: break-word;
}
.q-answer.collapsed {
  display: -webkit-box; -webkit-line-clamp: 3;
  -webkit-box-orient: vertical; overflow: hidden;
  white-space: normal;  /* line-clamp와 pre-wrap 병용 불가 */
}
.q-answer-toggle {
...
/* ── 멀티턴 서브행 ── */
.mt-turns { margin-top: 0; }
.mt-turn-row {
  display: grid;
  grid-template-columns: 28px 1fr auto;  /* 턴번호 | 본문 | 상품버튼 */
  gap: 0 10px; padding: 8px 0; align-items: start;
  border-top: 1px dashed var(--line);
}
.mt-turn-row:first-child { border-top: none; padding-top: 0; }
.mt-turn-num {
...
.mt-turn-body { min-width: 0; }
.mt-turn-product { padding-top: 2px; white-space: nowrap; }
...
**데이터 준비**
- [ ] 단일턴 `product_scores` → `PRODUCTS_DB["Q001"]` 형태로 직렬화
- [ ] 멀티턴 각 턴 `product_scores` → `PRODUCTS_DB["MT001-T1"]` 형태로 직렬화 (`case_id` 키)
- [ ] `answer_text` / `keyword_relevance_check.keywords` → HTML 생성 시 셀에 직접 삽입

**단일턴 표**
- [ ] 헤더: `질의 / 답변 · 액션칩` 컬럼명
- [ ] 셀 클래스: `class="q-cell"`
- [ ] 케이스 셀: `query_id` 아래 `session_id`를 소형 모노스페이스로 표시
- [ ] ① `<div class="q-text">` — `query`
- [ ] ② `<div class="q-chips">` — `keywords` → `<span class="q-chip">` 반복, 빈 배열이면 div 생략
- [ ] ③ `<div class="q-answer collapsed" id="ans-{query_id}">` — `answer_text`, 없으면 생략
- [ ] ③ `<span class="q-answer-toggle">▾ 더보기</span>` — 답변 있을 때만
- [ ] `answer_text` HTML 특수문자 이스케이프 (`&` `<` `>`)
- [ ] 상품 컬럼: `product_scores.length > 0` → `상품 N건` / `= 0` → disabled `0건`

**멀티턴 표**
- [ ] 헤더: `턴별 질의 / 답변 · 액션칩` 컬럼명 (턴 수/상품 컬럼 없음)
- [ ] 시나리오 1행 안에 `<div class="mt-turns">` 래퍼
- [ ] 케이스 셀: `query_id` 아래 `session_id`를 소형 모노스페이스로 표시
- [ ] 각 턴 → `<div class="mt-turn-row">` (grid: 28px 턴번호 | 1fr 본문 | auto 상품버튼)
- [ ] `<span class="mt-turn-num">T{turn_number}</span>`
- [ ] 본문: `.mt-turn-body` 안에 q-text / q-chips / q-answer 동일 구조 (id = `ans-{case_id}`)
- [ ] 상품 버튼: `.mt-turn-product` 안, `openProducts('{case_id}', ...)` 호출
- [ ] 0건 턴: disabled `0건` 버튼
