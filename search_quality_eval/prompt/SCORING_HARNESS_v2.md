...
| **상품** | `product_scores` | 상품 뷰어 버튼 ← 7-3 참조 |
| **답변** | `answer_text` + `keyword_relevance_check.keywords` | 답변 뷰어 버튼 ← 7-4 참조 |

**멀티턴 표**는 `턴` 컬럼이 n 대신 들어가며, 상품/답변 버튼은 `—`로 표시한다.

### 7-3. 상품 뷰어 (Products Viewer)
...
### 7-4. 답변 뷰어 (Answer Viewer) ← NEW

에이전트 답변 텍스트와 액션칩을 모달로 확인한다.

#### 버튼 규칙
- 단일턴 모든 케이스: **`답변`** 버튼 (초록/teal 계열 테두리, 클릭 가능)
- 멀티턴 케이스: **`—`** (버튼 없음)

#### JSON 소스 매핑

| 표시 항목 | report JSON 필드 |
|---|---|
| 답변 텍스트 | `answer_text` |
| 액션칩 목록 | `keyword_relevance_check.keywords` (배열) |

#### 모달 구성

```
┌──────────────────────────────────────┐
│ [Q001]  初心者向けのジョギングシューズは…  [✕] │  ← 헤더
├──────────────────────────────────────┤
│  답변 254자  액션칩 4개               │  ← 통계 바
├──────────────────────────────────────┤
│  # ウエーブライダー  # マキシマイザー      │  ← 액션칩 행
│  # 幅広設計  # 10,000円以下           │
├──────────────────────────────────────┤
│  ランニングを始められる方にぴったりの      │  ← 답변 텍스트
│  シューズをいくつかご用意しました。       │  (pre-wrap, 스크롤)
│  …                                   │
└──────────────────────────────────────┘
```

**모달 헤더**: cid 뱃지 + 쿼리(최대 50자) + ✕ 닫기

**통계 바**: 답변 글자수 / 액션칩 개수

**액션칩 영역**: `keyword_relevance_check.keywords` 배열 → pill 형태 태그, `#` 접두어
- 칩이 없으면 영역 자체 숨김

**답변 텍스트 영역**: `answer_text` 전문, `white-space: pre-wrap`, 최대 높이 52vh + 스크롤
- 답변 없으면 "📭 답변 텍스트 없음" 안내
...
세 개의 JS 상수를 `<script>` 블록에 인라인으로 주입한다.
...
// 답변 뷰어용
const ANSWERS_DB = {
  "Q001": "ランニングを始められる方にぴったりのシューズを…",
  "Q002": "…",
};

// 액션칩용
const CHIPS_DB = {
  "Q001": ["初心者向け定番 ウエーブライダー", "軽量で履きやすい マキシマイザー", …],
  "Q002": ["防水機能付きウォーキングシューズ", …],
};
</script>
```

**키**: `query_id` 문자열 ("Q001", "MT001" 등)  
**멀티턴**: `PRODUCTS_DB` / `ANSWERS_DB` / `CHIPS_DB` 포함 여부 무방 (버튼이 `—`이므로 참조 안 됨)

버튼 `onclick` 패턴:

```html
<!-- 상품 버튼 -->
<button class="btn-products" onclick="openProducts('Q001', &quot;쿼리텍스트&quot;)">상품 13건</button>

<!-- 답변 버튼 -->
<button class="btn-answer" onclick="openAnswer('Q001', &quot;쿼리텍스트&quot;)">답변</button>
```

### 7-6. 스타일 가이드 (CSS 변수 기반)

기존 레포트 디자인 토큰을 그대로 사용한다.
...
.btn-answer {
  ...
  background: transparent; color: var(--accent1); cursor: pointer;
}
.btn-answer:hover { background: var(--accent1); color: #fff; }

/* 액션칩 태그 */
.answer-chip-tag {
  padding: 4px 11px; border-radius: 999px;
  font-size: 11.5px; font-weight: 500;
  background: var(--accent2); color: #fff; opacity: 0.88;
}
...
/* 답변 모달은 더 좁게 */
#answerModal .modal-box { width: min(700px, 94vw); }
```
...
- [ ] `answer_text` → `ANSWERS_DB` JS 상수로 직렬화
- [ ] `keyword_relevance_check.keywords` → `CHIPS_DB` JS 상수로 직렬화

**단일턴 표 컬럼**
- [ ] **상품** 컬럼: `product_scores.length > 0` → `상품 N건` 버튼 / `= 0` → disabled `0건` 버튼
- [ ] **답변** 컬럼: 모든 단일턴 케이스에 `답변` 버튼 (answer_text 유무 무관)
- [ ] 멀티턴 표: 상품/답변 컬럼 모두 `—` 렌더링
...
- [ ] JSON 패널: 토글 가능한 `<pre>` 코드 블록
- [ ] 0건 케이스: 📭 안내 메시지

**답변 모달 (openAnswer)**
- [ ] 헤더: cid 뱃지 + 쿼리(50자) + ✕
- [ ] 통계 바: 답변 글자수 + 액션칩 개수
- [ ] 액션칩 영역: CHIPS_DB[cid] → pill 태그 렌더링, 없으면 영역 숨김
- [ ] 답변 텍스트: pre-wrap, 최대 높이 52vh + 스크롤
- [ ] 빈 답변: 📭 안내 메시지

**공통**
- [ ] 닫기: ✕ 버튼 / 배경 클릭 / ESC 키 (두 모달 모두)
- [ ] 모바일 대응: `grid-template-columns: 1fr` (@media max-width: 600px)
